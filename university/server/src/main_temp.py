...
from src.db import Student, Professor, Department, Group, get_db, Subject
from src.models import DepartmentModel, ProfessorModel, StudentModel, SubjectModel
...
@app.post("/subjects/")
async def create_subject(subject: SubjectModel, db: AsyncSession = Depends(get_db)):
    new_subject = Subject(
        name=subject.name
    )
    db.add(new_subject)
...
async def update_subject(subject_id: int, new_name: str, db: AsyncSession = Depends(get_db)):
    subject = await db.get(Subject, subject_id)
    if not subject:
        raise HTTPException(status_code=404, detail="Subject not found")
...
async def rebalance_groups(db: AsyncSession, department_name: str):
    ...
    groups_count = max(math.ceil(len(students)/10), min(len(professors), len(students)))
    students_avg = len(students)//groups_count
    professor_groups_avg = len(professors)//groups_count

    groups = {}
    # for i in range()
    # for student in students:
    #     student_note = await db.get(Professor, student.)
    #
    #     student_note.name = professor_update.name or professor.name
